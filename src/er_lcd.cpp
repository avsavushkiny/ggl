...
void SPIWrite_byte(uint8_t dat)
{
    ...
    for (int i = 0; i < 8; i++)
    {  
        if ((dat & 0x80) ) digitalWrite(LCD_MOSI, HIGH); 
...
void er_lcd_char(unsigned char x, unsigned char y, char acsii, char size, char mode, uint8_t* buffer)
{
    ...
    for(i = 0;i<size;i++) {
        ...
        for(j =0;j<8;j++)
        {
            if(temp & 0x80) er_lcd_pixel(x, y, 1, buffer);
            else er_lcd_pixel(x, y, 0, buffer);
            temp <<= 1;
            y++;
            if((y-y0) == size)
            {
		...
		for(i = 0;i <chWidth;i++){
			if(pgm_read_byte(pBmp + j * byteWidth + i / 8) & (128 >> (i & 7))){
				er_lcd_pixel(x + i,y + j, 1, buffer);
                        ...
                        else er_lcd_pixel(x + i,y + j, 0, buffer);
		}
	}		
}
...
{
	uint16_t i, j,k; uint8_t page=chHeight*2/8;
...
void demo_sine(uint8_t* buffer)
{
  ...
  drawSine(HEIGHT/2, HEIGHT/4, 5, 1,buffer);
   er_lcd_display(buffer);   delay(1000);  
...
void fillTriangle ( int16_t x0, int16_t y0,int16_t x1, int16_t y1,int16_t x2, int16_t y2, uint16_t color,uint8_t* buffer) 
 {
  ...
  if (y0 > y1) {
    swapxy(y0, y1); swapxy(x0, x1);
  }
  if (y1 > y2) {
    swapxy(y2, y1); swapxy(x2, x1);
  }
  if (y0 > y1) {
    swapxy(y0, y1); swapxy(x0, x1);
  }
...
void testLines(uint8_t* buffer) 
{
  ...
  for(x2=0; x2<w; x2+=6) {drawLine(x1, y1, x2, y2, color, buffer);  er_lcd_display(buffer);   delay(30); }
  x2    = w - 1;
  for(y2=0; y2<h; y2+=6) {drawLine(x1, y1, x2, y2, color, buffer);  er_lcd_display(buffer);   delay(30); }
...
void testFastLines(uint8_t* buffer)
{
  ...
  for(y=0; y<h; y+=5) {drawFastHLine(0, y, w, 1, buffer);  er_lcd_display(buffer);  delay(30); }
  for(x=0; x<w; x+=5) {drawFastVLine(x, 0, h, 1, buffer);  er_lcd_display(buffer);  delay(30); }
...
void testRects(uint8_t* buffer)
{
  ...
  for(i=2; i<n; i+=6) {
    i2 = i / 2;
    drawRect(cx-i2, cy-i2, i, i, 1, buffer);  er_lcd_display(buffer);  delay(150); 
  }
...
void testFilledRects(uint8_t* buffer)
{
  ...
  for(i=n; i>0; i-=6) {
    ...
    fillRect(cx-i2, cy-i2, i, i, 1, buffer);  er_lcd_display(buffer);  delay(150);   er_lcd_clear(buffer);
...
void testFilledCircles(uint8_t radius, uint8_t* buffer)
{
  ...
  for(x=radius; x<w; x+=r2) {
    for(y=radius; y<h; y+=r2) {
      fillCircle(x, y, radius, 1, buffer);  er_lcd_display(buffer);  delay(50); 
    }
  }
...
void testCircles(uint8_t radius,uint8_t* buffer) 
{
  ...
  for(x=radius; x<w; x+=r2) {
    for(y=radius; y<h; y+=r2) {
      drawCircle(x, y, radius, 1, buffer);  er_lcd_display(buffer);  delay(50); 
    }
  }
...
void testTriangles(uint8_t* buffer) 
{
  ...
  for(i=0; i<n; i+=5) {
    drawTriangle(
      ...
      1, buffer);  er_lcd_display(buffer);  delay(150); 
  }
...
void testFilledTriangles(uint8_t* buffer) 
{
  ...
  for(i=min(cx,cy); i>10; i-=5) {
    ...
    fillTriangle(cx, cy - i, cx - i, cy + i, cx + i, cy + i,1, buffer);  er_lcd_display(buffer);  delay(150);  er_lcd_clear(buffer); 
...
void testRoundRects(uint8_t* buffer) 
{
  ...
  for(i=0; i<w; i+=6) {
    i2 = i / 2;
    drawRoundRect(cx-i2, cy-i2, i, i, i/8, 1, buffer);  er_lcd_display(buffer);  delay(150); 
  }
...
void testFilledRoundRects(uint8_t* buffer)
{
  ...
  for(i=min(WIDTH, HEIGHT); i>20; i-=6) {
    i2 = i / 2;
    fillRoundRect(cx-i2, cy-i2, i, i, i/8, 1, buffer);  er_lcd_display(buffer);  delay(150);   er_lcd_clear(buffer);  
  }
